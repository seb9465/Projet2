...
export abstract class GrilleAbs implements OnDestroy {
    ...
    public manageKeyEntry(event: KeyboardEvent): void {
        if (event.key === "Backspace") {
            this.focus.focusOnPreviousLetter(
                this.motSelectionne,
                this.lockedLetter
            );
        } else if (
            event.key.toUpperCase().charCodeAt(0) >= CONST.KEYCODE_MIN &&
            event.key.toUpperCase().charCodeAt(0) <= CONST.KEYCODE_MAX
        ) {
    ...
    private remplirPositionLettresMot(leMot: Mot): void {
        ...
        for (let i: number = 1; i < leMot.longueur; i++) {
            leMot.estVertical
                ? (tmp = this.makeID(x, y + i, ""))
                : (tmp = this.makeID(x + i, y, ""));
    ...
    protected validateWord(): void {
        ...
        if (valid) {
            ...
            this._servicePointage.incrementationNbMotDecouv(
                CONST.INCR_UN_MOT_DECOUVERT
            );
    ...
    protected lockLettersFromWord(): void {
        for (let i: number = 0; i < this.motSelectionne.longueur; i++) {
            if (this.motSelectionne.estVertical) {
                this.lockedLetter[this.motSelectionne.premierX][
                    this.motSelectionne.premierY + i
                ] = true;
            } else {
                this.lockedLetter[this.motSelectionne.premierX + i][
                    this.motSelectionne.premierY
                ] = true;
            }
        }
    }
    ...
    protected createWordFromSelectedLetters(): string {
        ...
        for (const elem of this.motSelectionne.positionsLettres) {
            wordCreated += (document.getElementById(elem) as HTMLInputElement)
                .value;
        }
